...
// Funzione per valutare le mani di poker
const evaluatePokerHand = (hand) => {
  const values = hand.map(card => card.value).sort((a, b) => b - a);
  const suits = hand.map(card => card.suit);
  
  // Controlla il colore (Flush)
  const isFlush = suits.every(suit => suit === suits[0]);
  
  // Controlla la scala (Straight)
  const isStraight = values.every((val, i) => i === 0 || val === values[i - 1] - 1);
  
  // Conta le occorrenze dei valori per coppie, tris, ecc.
  const valueCounts = {};
  values.forEach(val => {
  ...
  if (isFlush && isStraight) return { rank: 8, description: 'Straight Flush' }; // Scala reale
  if (counts[0] === 4) return { rank: 7, description: 'Four of a Kind' }; // Poker
  if (counts[0] === 3 && counts[1] === 2) return { rank: 6, description: 'Full House' }; // Full
  if (isFlush) return { rank: 5, description: 'Flush' }; // Colore
  if (isStraight) return { rank: 4, description: 'Straight' }; // Scala
  if (counts[0] === 3) return { rank: 3, description: 'Three of a Kind' }; // Tris
  if (counts[0] === 2 && counts[1] === 2) return { rank: 2, description: 'Two Pair' }; // Doppia coppia
  if (counts[0] === 2) return { rank: 1, description: 'One Pair' }; // Coppia
...
// Componente per il tavolo centrale, ora dedicato al Poker PvP
...
const [gamePhase, setGamePhase] = useState('pre-flop'); // Fase del gioco
const [opponentCardsVisible, setOpponentCardsVisible] = useState(false); // Visibilità carte avversario



const advanceGamePhase = async (nextTurn) => {
  if (gamePhase === 'pre-flop') {
    setPokerMessage('The dealer is dealing the Flop...');
    await new Promise(resolve => setTimeout(resolve, 1000));
    dealCommunityCards(3); // Flop
    setGamePhase('flop');
    setPokerMessage('Flop: Place your bets.');
  } else if (gamePhase === 'flop') {
    setPokerMessage('The dealer is dealing the Turn...');
    await new Promise(resolve => setTimeout(resolve, 1000));
    dealCommunityCards(1); // Turn
    setGamePhase('turn');
    setPokerMessage('Turn: Place your bets.');
  } else if (gamePhase === 'turn') {
    setPokerMessage('The dealer is dealing the River...');
    await new Promise(resolve => setTimeout(resolve, 1000));
    dealCommunityCards(1); // River
    setGamePhase('river');
    setPokerMessage('River: Place your bets.');
  ...
  // Funzione per aggiornare le missioni
  ...
  // Funzione per unirsi alla partita di Poker PvP
  const joinPokerGame = async () => {
    ...
    const newPlayer = { address: publicKey.toString(), bet: betAmount };
    setPokerPlayers((prev) => {
      const updatedPlayers = [...prev, newPlayer];
      if (updatedPlayers.length === 2) {
        startPokerGame(updatedPlayers);
      }
  ...
  const startPokerGame = async (players) => {
    setPokerStatus('playing');
    setPokerMessage('The dealer is dealing the cards...');
  
    // Simula il dealer che dà le carte private
    await new Promise(resolve => setTimeout(resolve, 1000)); // Delay per simulare l'animazione
    const player1Cards = [drawCard(), drawCard()];
    const player2Cards = [drawCard(), drawCard()];
    setPokerPlayerCards({
      [players[0].address]: player1Cards,
      [players[1].address]: player2Cards,
    });
    ...
    try {
      ...
      await connection.confirmTransaction(signature);
      setCurrentTurn(players[0].address);
      setPokerPot(betAmount * 2);
      setPlayerBets({
        [players[0].address]: betAmount,
        [players[1].address]: betAmount,
      });
      setCurrentBet(betAmount);
      setGamePhase('pre-flop');
      setPokerMessage('Pre-Flop: Place your bets.');
    } catch (err) {
  ...
  // Distribuisci carte comuni
  const dealCommunityCards = (count) => {
    const newCards = Array(count).fill().map(() => drawCard());
    setPokerTableCards((prev) => [...prev, ...newCards]);
  };

  const makePokerMove = async (move, amount = 0) => {
    if (pokerStatus !== 'playing' || currentTurn !== publicKey.toString()) {
      setPokerMessage("It's not your turn or the game is not in progress!");
      return;
    }
  
    const playerAddress = publicKey.toString();
    const opponent = pokerPlayers.find((p) => p.address !== playerAddress);
    const currentPlayerBet = playerBets[playerAddress] || 0;
  
    if (move === 'fold') {
      setPokerStatus('finished');
      setOpponentCardsVisible(true);
      setPokerMessage(`${opponent.address.slice(0, 8)}... wins! You folded.`);
      await distributePokerWinnings(opponent.address);
    } else if (move === 'check') {
      if (currentBet > currentPlayerBet) {
        setPokerMessage('You cannot check, you must call or raise!');
      } else {
        setPokerMessage('You checked.');
        advanceGamePhase(opponent.address);
      }
    } else if (move === 'call') {
      const amountToCall = currentBet - currentPlayerBet;
      if (amountToCall > 0) {
        const betInLamports = amountToCall * LAMPORTS_PER_SOL;
        const transaction = new Transaction().add(
          SystemProgram.transfer({
        ...
        try {
          ...
          const signature = await connection.sendRawTransaction(signed.serialize());
          await connection.confirmTransaction(signature);
          setPokerPot(prev => prev + amountToCall);
          setPlayerBets(prev => ({ ...prev, [playerAddress]: currentBet }));
          setPokerMessage(`You called ${amountToCall.toFixed(2)} SOL.`);
        } catch (err) {
          console.error('Bet error:', err);
          setPokerMessage('Bet failed. Please try again.');
          return;
        ...
        setPokerMessage('You called (no additional bet).');
      }
      advanceGamePhase(opponent.address);
    } else if (move === 'bet' || move === 'raise') {
      const newBet = move === 'bet' ? amount : currentBet + amount;
      if (newBet <= currentBet) {
        setPokerMessage('The bet must be higher than the current bet!');
        return;
      }
      if (validateBet(newBet)) {
        setPokerMessage(validateBet(newBet));
        return;
      }
      ...
      try {
        ...
        const signature = await connection.sendRawTransaction(signed.serialize());
        await connection.confirmTransaction(signature);
        setPokerPot(prev => prev + additionalBet);
        setPlayerBets(prev => ({ ...prev, [playerAddress]: newBet }));
        setCurrentBet(newBet);
        setPokerMessage(`You ${move === 'bet' ? 'bet' : 'raised'} ${additionalBet.toFixed(2)} SOL.`);
        setCurrentTurn(opponent.address);
      } catch (err) {
        console.error('Bet error:', err);
        setPokerMessage('Bet failed. Please try again.');
      }
  ...
  const endPokerGame = async () => {
    setPokerStatus('finished');
    setOpponentCardsVisible(true);
    const playerHand = [...pokerPlayerCards[publicKey.toString()], ...pokerTableCards];
    const opponentHand = [...pokerPlayerCards[pokerPlayers.find(p => p.address !== publicKey.toString()).address], ...pokerTableCards];
    const playerEvaluation = evaluatePokerHand(playerHand);
    const opponentEvaluation = evaluatePokerHand(opponentHand);
  
    let winner;
    if (playerEvaluation.rank > opponentEvaluation.rank) {
      winner = pokerPlayers.find(p => p.address === publicKey.toString());
      setPokerMessage(`You win with a ${playerEvaluation.description}!`);
    } else if (opponentEvaluation.rank > playerEvaluation.rank) {
      winner = pokerPlayers.find(p => p.address !== publicKey.toString());
      setPokerMessage(`${winner.address.slice(0, 8)}... wins with a ${opponentEvaluation.description}!`);
    } else {
      // In caso di pareggio, confronta la carta più alta
      const playerHighCard = Math.max(...playerHand.map(card => card.value));
      const opponentHighCard = Math.max(...opponentHand.map(card => card.value));
      if (playerHighCard > opponentHighCard) {
        winner = pokerPlayers.find(p => p.address === publicKey.toString());
        setPokerMessage(`You win with a higher card (${playerHighCard})!`);
      } else if (opponentHighCard > playerHighCard) {
        winner = pokerPlayers.find(p => p.address !== publicKey.toString());
        setPokerMessage(`${winner.address.slice(0, 8)}... wins with a higher card (${opponentHighCard})!`);
      } else {
        setPokerMessage('It\'s a tie! The pot is split.');
        // Dividi il piatto (per semplicità, assegniamo al primo giocatore)
        winner = pokerPlayers[0];
      }
    }
    await distributePokerWinnings(winner.address);
  };




  // Calcolo del punteggio (semplificato per Texas Hold'em)
  const calculatePokerScore = (cards) => {
    return cards.reduce((sum, card) => sum + card.value, 0);
  };

  // Distribuzione delle vincite
  const distributePokerWinnings = async (winnerAddress) => {
    const winAmountInLamports = pokerPot * LAMPORTS_PER_SOL;
    const transaction = new Transaction().add(
      SystemProgram.transfer({
        fromPubkey: wallet.publicKey,
  ...
  <div>
    ...
    <div className="game-box p-6 mb-10">
      ...
      ) : pokerStatus === 'playing' && currentTurn === publicKey?.toString() ? (
      ...
      <button onClick={() => setSelectedGame(null)} className="w-full casino-button mt-4">
        Back to Casino Floor
      </button>
    </div>
  </div>
