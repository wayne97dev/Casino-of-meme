...
import {
  getMint,
  TOKEN_PROGRAM_ID,
  TOKEN_2022_PROGRAM_ID, // Aggiungi questa riga
  AccountLayout,
  ...
  const [selectedGame, setSelectedGame] = useState(null);
    ...
    const fetchComBalance = async () => {
      if (!connected || !publicKey) {
        console.log('DEBUG - Wallet not connected or publicKey missing:', { connected, publicKey });
        setComBalance(0);
        return;
      }
      ...
      try {
        ...
        const response = await fetch(`${BACKEND_URL}/com-balance/${publicKey.toString()}`, {
          ...
          credentials: 'include', // Includi i cookie per problemi di CORS
        });
        ...
        if (result.success) {
          ...
          throw new Error(result.error || 'Failed to fetch COM balance');
        }
      } catch (err) {
...
useEffect(() => {
  ...
  const handleDistributeWinnings = async ({ winnerAddress, amount, isRefund }) => {
    ...
    if (winnerAddress === publicKey?.toString()) {
      try {
        // Validazione iniziale di amount
        if (!amount || isNaN(amount) || amount <= 0) {
          throw new Error('Invalid winnings amount');
        }
  
        // Recupera le informazioni sulle transfer fee per mostrare l'importo netto
        const connection = new Connection(RPC_ENDPOINT, 'confirmed');
        console.log('DEBUG - Fetching mint account info...');
        ...
        let decimals = 6; // Valore di fallback
        try {
          ...
          if (!mintInfo || typeof mintInfo.decimals !== 'number') {
            throw new Error('Invalid mint info: decimals not found');
          }
          decimals = mintInfo.decimals;
          console.log('DEBUG - Mint decimals:', decimals);
        } catch (err) {
          console.error('DEBUG - Error fetching mint info, using default decimals:', err.message);
        }
        ...
        if (transferFeeConfig && transferFeeConfig.state &&
          ...
          netAmount = Number(amountInBaseUnits - feeAmount) / Math.pow(10, decimals);
          console.log('DEBUG - Transfer fee for winnings:', { feeAmount: feeAmount.toString(), netAmount });
        } else {
        ...
        if (response.ok && result.success) {
          ...
          if (!isRefund) {
            setPlayerStats(prev => ({
              ...prev,
              wins: prev.wins + 1,
              totalWinnings: prev.totalWinnings + netAmount,
            }));
          }
          setPokerMessage(`You ${isRefund ? 'received a refund of' : 'won'} ${netAmount.toFixed(2)} ${TOKEN_SYMBOL} (after ${feeAmount > 0 ? (Number(feeAmount) / Math.pow(10, decimals)).toFixed(2) : 0} ${TOKEN_SYMBOL} transfer fee)!`);
          fetchComBalance();
        } else {
          ...
          setPokerMessage(
            `Failed to receive ${amount.toFixed(2)} ${TOKEN_SYMBOL}: ${errorMessage}. Transaction Signature: ${transactionSignature}. Please contact support with these details.`
          );
        }
      } catch (err) {
        console.error('DEBUG - Error distributing winnings:', err.message, err.stack);
        setPokerMessage(
          `Error receiving ${amount.toFixed(2)} ${TOKEN_SYMBOL}: ${err.message}. Please contact support with error details.`
        );
      }
    } else {
      setPokerMessage(`${winnerAddress.slice(0, 8)}... has ${isRefund ? 'received a refund of' : 'won'} ${amount.toFixed(2)} ${TOKEN_SYMBOL}!`);
    }
  };


  const handleRefund = async ({ message, amount, isRefund }) => {
    console.log('DEBUG - Refund event received:', { message, amount, isRefund });
    setPokerMessage(message);
  
    if (!connected || !publicKey || !amount || isNaN(amount) || amount <= 0) {
      console.log('DEBUG - Cannot process refund: Invalid parameters:', { connected, publicKey, amount });
    ...
    try {
      const connection = new Connection(RPC_ENDPOINT, 'confirmed');
      console.log('DEBUG - Fetching mint info for transfer fee calculation...');
      const accountInfo = await connection.getParsedAccountInfo(new PublicKey(MINT_ADDRESS));
      if (!accountInfo.value) {
        throw new Error('Failed to fetch mint account info');
      }
      ...
      if (transferFeeConfig && transferFeeConfig.state &&
        ...
        feeAmount = calculatedFee < maxFee ? calculatedFee : maxFee;
        netAmount = Number(amountInBaseUnits - feeAmount) / Math.pow(10, decimals);
        console.log('DEBUG - Transfer fee for refund:', { feeAmount: feeAmount.toString(), netAmount });
      } else {
        console.log('DEBUG - No valid TransferFee extension found, proceeding without fee');
      }
  
      console.log('DEBUG - Sending request to /refund:', { playerAddress: publicKey.toString(), amount });
      ...
      if (!response.ok) {
        ...
        console.error('DEBUG - Refund request failed:', { status: response.status, errorText });
        throw new Error(`HTTP error! Status: ${response.status} - ${errorText}`);
      }
  
      const result = await response.json();
      console.log('DEBUG - Response from /refund:', result);
      if (result.success) {
        setPokerMessage(`Refund of ${netAmount.toFixed(2)} ${TOKEN_SYMBOL} received (after ${feeAmount > 0 ? (Number(feeAmount) / Math.pow(10, decimals)).toFixed(2) : 0} ${TOKEN_SYMBOL} transfer fee)!`);
        fetchComBalance();
        if (!isRefund) {
          setPlayerStats(prev => ({
            ...prev,
        ...
        console.error('DEBUG - Refund failed:', result.error);
        setPokerMessage(`Refund failed: ${result.error || 'Error processing refund'}. Contact support.`);
      }
    } catch (err) {
      console.error('DEBUG - Error processing refund:', err.message, err.stack);
      setPokerMessage(`Refund failed: ${err.message}. Contact support.`);
    }
...
const createAndSignTransaction = async (betAmount, gameType, additionalData = {}) => {
  ...
  const joinPokerGame = async () => {
    ...
    try {
      ...
      // Recupera le informazioni del mint per ottenere i decimali
      const mintInfo = await getMint(connection, new PublicKey(MINT_ADDRESS), 'confirmed', TOKEN_2022_PROGRAM_ID);
      ...
      // Usa createTransferCheckedInstruction invece di createTransferInstruction
      transaction.add(
        createTransferCheckedInstruction(
          userATA, // Source ATA
          new PublicKey(MINT_ADDRESS), // Mint
          casinoATA, // Destination ATA
          publicKey, // Owner
          Math.round(betAmount * Math.pow(10, decimals)), // Importo in unità base (con decimali)
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(`Failed to join game: HTTP ${response.status} - ${errorText}`);
      }
      ...
      if (result.success) {
        socket.emit('joinGame', {
          ...
          if (ack) {
            setPokerMessage('Ti sei unito al gioco!');
            fetchComBalance();
          } else {
        ...
        setPokerMessage(`Impossibile unirsi al gioco: ${result.error || 'Errore sconosciuto'}`);
      }
    } catch (err) {
      console.error('DEBUG - Error joining poker game:', err.message, err.stack);
  ...
  const getHolders = async (mintAddress, connection) => {
    console.log('DEBUG - Starting getHolders for mint:', mintAddress);
    const holders = [];
    const filters = [
      { memcmp: { offset: 0, bytes: mintAddress } }, // Filtra per mint address
    ];
  
    try {
      console.log('DEBUG - Fetching token accounts with filters:', JSON.stringify(filters));
      ...
      console.log('DEBUG - Sample accounts:', accounts.slice(0, 3).map(acc => ({
      ...
      for (const account of accounts) {
        try {
          // Verifica che l'account appartenga al programma corretto
          if (account.account.owner.toBase58() !== TOKEN_2022_PROGRAM_ID.toBase58()) {
            console.warn('DEBUG - Skipping account with invalid owner:', {
          ...
          // Decodifica l'account token
          const accountData = AccountLayout.decode(account.account.data);
          const amount = Number(accountData.amount) / 1e6; // Converti in unità leggibili (decimals = 6 per COM)
          ...
          // Verifica che l'account abbia un saldo positivo e un owner valido
          if (amount > 0 && accountData.owner && accountData.mint.toBase58() === mintAddress) {
            holders.push({
              address: accountData.owner.toBase58(),
              amount,
            });
          } else {
            console.log('DEBUG - Skipping account:', {
              pubkey: account.pubkey.toBase58(),
              amount,
              owner: accountData.owner ? accountData.owner.toBase58() : 'null',
              mint: accountData.mint ? accountData.mint.toBase58() : 'null',
              reason: amount <= 0 ? 'Zero balance' : 'Invalid owner or mint',
            });
          }
        } catch (err) {
          console.warn('DEBUG - Error decoding account:', {
            pubkey: account.pubkey.toBase58(),
            error: err.message,
          });
        }
      ...
      // Ordina gli holders per quantità decrescente
      const sortedHolders = holders.sort((a, b) => b.amount - a.amount);
      console.log('DEBUG - Holders fetched:', sortedHolders.length);
      console.log('DEBUG - Top 5 holders:', sortedHolders.slice(0, 5).map(h => ({
        address: h.address,
        amount: h.amount,
      })));
  
      if (sortedHolders.length === 0) {
        console.log('DEBUG - No valid holders found. Possible reasons:');
        console.log('  - No accounts hold this token.');
        console.log('  - RPC node issues or incorrect mint address:', mintAddress);
        console.log('  - Filters or program mismatch (expected TOKEN_2022_PROGRAM_ID):', TOKEN_2022_PROGRAM_ID.toBase58());
        console.log('  - Check Solscan for holders: https://solscan.io/token/', mintAddress);
      } else {
        console.log('DEBUG - Successfully fetched holders:', sortedHolders.map(h => ({
          address: h.address,
          amount: h.amount,
        })));
      }
      ...
      console.error('DEBUG - Error fetching token accounts:', err.message, err.stack);
      console.log('DEBUG - Possible causes:');
      console.log('  - RPC connection failure:', RPC_ENDPOINT);
      console.log('  - Invalid mint address:', mintAddress);
      console.log('  - Program mismatch (expected TOKEN_2022_PROGRAM_ID):', TOKEN_2022_PROGRAM_ID.toBase58());
      return [];
    }
  };
...
{connected && publicKey && (
  <div className="wallet-info-box">
    <p className="text-orange-700">
      Your COM Balance: {TOKEN_SYMBOL} {comBalance.toFixed(6)}
      <button onClick={fetchComBalance} className="casino-button ml-2">
        Refresh
      </button>
    </p>
  </div>
